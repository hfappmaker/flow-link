...
import { saveWorkPreference } from "@/lib/actions";
import { requireFreelancerProfile } from "@/lib/page-guards";
import { locationModeLabel, workPreferenceCompleteness } from "@/lib/utils";
...
export default async function WorkPreferencesPage() {
  const { user, profile } = await requireFreelancerProfile({
    ...
    include: {
      workPreference: true,
      savedJobSearches: { orderBy: { createdAt: "desc" } },
    },
  });
          ...
          {profile.savedJobSearches.length > 0 ? (
            <div className="grid gap-3 md:grid-cols-2">
              {profile.savedJobSearches.map((search) => (
                <Link className="rounded-md border border-stone-200 bg-white p-4 shadow-sm transition hover:border-emerald-300" href={savedSearchHref(search)} key={search.id}>
                  ...
                  <p className="mt-3 text-sm font-semibold text-emerald-700">このフィードを見る</p>
                </Link>
              ))}
            </div>
          ) : (
...
function savedSearchSummary(search: SavedSearch) {
  return [
    ...
    search.notificationCadence && `通知: ${search.notificationCadence}`,
